...
# add features of a legitimate or hijacker
# select the 1 column which contains the legitimate ASes
legitimate = new_data_CAIDA.iloc[:, [0]]
# select the 2 column which contains the hijacked ASes
hijacker = new_data_CAIDA.iloc[:, [1]]
...
for index, row in new_data_CAIDA.iterrows():
    ...
    # where row[0] == legitimate and row[1] = hijacker
    columns_names = columns_names.insert(len(columns_names), str(row[0]))
    columns_names = columns_names.insert(len(columns_names), str(row[1]))
...
final_df.to_csv('legitimate_hijacker.csv')