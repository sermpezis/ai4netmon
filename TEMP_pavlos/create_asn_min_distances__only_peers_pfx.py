...
for msg in bgp.load_data_ripe_bgp_dumps(INPUT_STREAM):
   ...
   pathlen = len(path)
   origin_asn = path[-1]
   test_dict[int(origin_asn)][peer_ip] = min(pathlen, test_dict[int(origin_asn)].get(peer_ip,LARGE_PATH_LENGTH)) 
   

with open('test_asn2asn__only_peers_pfx_IPv6.json', 'w') as f:
